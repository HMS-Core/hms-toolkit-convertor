...
public class XEnumCreator {
     ...
     */
    public static void populateEnum(JClass def, ClassNode node) {
    ...
    private static void generateValues(JClass def, ClassNode node) {
        ...
        method.setModifiers(Arrays.asList(Modifier.PUBLIC.getName(), Modifier.STATIC.getName()));
        ...
        List<StatementNode> values = def.fields()
            .stream()
            .filter(it -> !it.isRedundant())
            .filter(it -> it.g().asJField().modifiers().containsAll(
                Arrays.asList(Modifier.STATIC.getName(), Modifier.FINAL.getName())))
            .filter(it -> it.g().asJField().type().equals(def.gName()))
            .map(it -> CallNode.create("get" + it.g().asJField().name(), Collections.emptyList()))
