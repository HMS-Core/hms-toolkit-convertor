...
/**
 * Class for XmsXhGenerator
 *
 * @since 2020-05-06
 */
public class XmsXhGenerator extends ModuleGenerator {
    private static final Logger LOGGER = LoggerFactory.getLogger(XmsXhGenerator.class);

    private String generatePath;

    public XmsXhGenerator(ProcessorUtils processorUtils) {
        ...
        modulePath = String.join(File.separator, targetPath, XmsConstants.XMS_MODULE_NAME,
            XmsConstants.XMS_SUBMODULE_NAME, "xh");
        ...
        kitSet.forEach(kit -> {
            if (allDepMap.get(kit) != null) {
                allDepMap.get(kit).forEach(dependency -> {
                    if (EnhancerUtils.isHmsType(dependency)) {
    ...
    boolean generateCode() {
        ...
        if (!kitSet.contains("Push")) {
            excludes.add("xms/xmsaux/xh/src/main/java/org/xms/f");
        }
        if (!kitSet.contains("Map")) {
            excludes.add("xms/xmsaux/xh/src/main/java/org/xms/g");
        }
        StaticPatcher.copyResourceDir(pluginPath, generatePath, "xms/xmsaux/xh", excludes);
        return true;
    }
